#!/usr/bin/env python3
"""Test UI interface by starting a minimal server without GRVT client"""
...
async def test_ui_server():
    ...
    print("🚀 Starting UI test server on port 9000...")

    # Start server in background
    config = uvicorn.Config(app, host="127.0.0.1", port=9000, log_level="info")
    server = uvicorn.Server(config)

    # Run server for a short time to test
    try:
        # Start server
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if "TW168 交易控制面板" in content:
                    print("✅ UI endpoint working - HTML content contains expected title")
                    print(f"📊 HTML content length: {len(content)} characters")
                else:
                    print("❌ UI endpoint returned HTML but missing expected content")
            else:
                print(f"❌ UI endpoint failed: {resp.status_code}")
        except Exception as e:
        ...
        try:
            resp = requests.post("http://127.0.0.1:9000/manual/signal", json=test_data, timeout=5)
            if resp.status_code == 200:
                result = resp.json()
                if result.get("ok"):
                    ...
                    print(f"❌ Manual signal endpoint returned error: {result}")
            else:
                print(f"❌ Manual signal endpoint failed: {resp.status_code}")
        except Exception as e:
            print(f"❌ Manual signal endpoint error: {e}")

        print("🎉 UI interface test completed successfully!")
        print("🌐 Access the UI at: http://127.0.0.1:9000")
