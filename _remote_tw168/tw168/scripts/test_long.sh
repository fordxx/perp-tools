...
echo "[2/3] ${NOW_HUMAN} ZONE=OVERSOLD (long filter)..."
curl -sS -X POST "${BASE_URL}/webhook/tradingview" \
  -H 'Content-Type: application/json' \
  -d @- <<JSON
{"secret":"${SECRET}","type":"ZONE","zone":"OVERSOLD","instId":"${INST_ID}","tf":"${TF}","t":"test_zone_${TS_ZONE}"}
JSON
echo

NOW_HUMAN="$(date '+%F %T')"
echo "[3/3] ${NOW_HUMAN} DIV (should open long in paper mode)..."
curl -sS -X POST "${BASE_URL}/webhook/tradingview" \
  -H 'Content-Type: application/json' \
  -d @- <<JSON
