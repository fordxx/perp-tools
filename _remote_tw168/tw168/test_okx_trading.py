#!/usr/bin/env python3
"""Test OKX trading flow: ladder orders → SL/TP → close position."""
...
# Add app to path
sys.path.insert(0, '/home/ubuntu/tw168')
...
def main():
    ...
    okx = OKXClient(base_url=SETTINGS.okx_base_url, creds=creds)
    ...
    level1_bps = SETTINGS.ladder_level1_bps  # 5
    level2_bps = SETTINGS.ladder_level2_bps  # 15
    level3_bps = SETTINGS.ladder_level3_bps  # 30

    level1_pct = SETTINGS.ladder_level1_pct  # 0.50
    level2_pct = SETTINGS.ladder_level2_pct  # 0.30
    level3_pct = SETTINGS.ladder_level3_pct  # 0.20

    # Calculate prices
    if TEST_SIDE == "buy":
        level1_px = last_price * (1 - level1_bps / 10000)
        level2_px = last_price * (1 - level2_bps / 10000)
        level3_px = last_price * (1 - level3_bps / 10000)
    else:
        level1_px = last_price * (1 + level1_bps / 10000)
        level2_px = last_price * (1 + level2_bps / 10000)
        level3_px = last_price * (1 + level3_bps / 10000)
    ...
    # Calculate sizes
    level1_sz = int(total_contracts * level1_pct)
    level2_sz = int(total_contracts * level2_pct)
    level3_sz = total_contracts - level1_sz - level2_sz

    print(f"Level 1: {level1_sz} contracts @ {level1_px:.4f} ({level1_pct:.0%})")
    print(f"Level 2: {level2_sz} contracts @ {level2_px:.4f} ({level2_pct:.0%})")
    print(f"Level 3: {level3_sz} contracts @ {level3_px:.4f} ({level3_pct:.0%})")
