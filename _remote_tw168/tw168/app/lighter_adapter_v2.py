...
class LighterClient(ExchangeClient):
    ...
    async def place_open_order(self, request: OrderRequest) -> Order:
        ...
        try:
            ...
            market_id, base_multiplier, price_multiplier = await self._get_market_config_async(
                request.symbol
            )
    ...
    async def place_close_order(self, position: Position, current_price: float) -> Order:
        ...
        try:
            # Get market configuration
            market_id, base_multiplier, price_multiplier = await self._get_market_config_async(position.order.symbol,
                timeout=10.0
            )
    ...
    async def place_stop_loss(self, symbol: str, side: str, size: float, trigger_price: float, limit_price: Optional[float] = None) -> Order:
        ...
        try:
            # Get market configuration
            market_id, base_multiplier, price_multiplier = await self._get_market_config_async(symbol,
                timeout=10.0
            )
    ...
    async def place_take_profit(self, symbol: str, side: str, size: float, trigger_price: float, limit_price: Optional[float] = None) -> Order:
    ...
    async def cancel_order(self, order_id: str, symbol: Optional[str] = None, market_id: Optional[int] = None) -> None:
        ...
        try:
            ...
            if market_id is None:
                ...
                market_id, _, _ = await self._get_market_config_async(symbol,
                    timeout=10.0
                )
    ...
    async def get_order_fills(self, *, inst_id: str, order_id: str) -> tuple[float, float] | None:
    ...
    async def enable_websocket(self, auto_subscribe_account: bool = True) -> None:
        ...
        try:
            ...
            if auto_subscribe_account and self._trading_enabled and self.account_index:
                await self._setup_account_subscriptions()
    ...
    async def subscribe_orderbook_stream(self, symbol: str, callback: Callable[[dict], None]) -> None:
    ...
    async def subscribe_trades_stream(self, symbol: str, callback: Callable[[dict], None]) -> None:
    ...
    async def subscribe_market_stats_stream(self, symbol: str, callback: Callable[[dict], None]) -> None:
