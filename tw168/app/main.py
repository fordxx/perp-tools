...
class ManualSignalRequest(BaseModel):
    ...
    entry_price: float | None = Field(default=None, description="指定入场价格（覆盖自动计算）")
    lever: str | None = Field(default=None, description="杠杆倍数，如 '10'")
    mgn_mode: str | None = Field(default=None, description="保证金模式: isolated 或 cross")
    risk_usdt: float | None = Field(default=None, description="每单风险金额 USDT，覆盖配置文件的 RISK_PER_TRADE_USDT")
    sl_price: float | None = Field(default=None, description="用户指定止损价格（覆盖自动计算）")
...
def _normalize_inst_id(raw_inst_id: str) -> str:
    ...
    if inst_id.endswith("USDT"):
        ...
        if base:
            return f"{base}-USDT-SWAP"
    # Plain ticker: BTC → BTC-USDT-SWAP
    if inst_id and "-" not in inst_id and "_" not in inst_id:
        return f"{inst_id}-USDT-SWAP"
    return inst_id
...
async def _calculate_order_size(
    *,
    ...
    entry_price: float,
    tf: str | None = None,
    risk_usdt_override: float | None = None,
    ...
    # Get timeframe-specific risk if available, otherwise use default; UI override takes priority
    risk_amount = risk_usdt_override if (risk_usdt_override and risk_usdt_override > 0) else (get_risk_per_trade(tf) if tf else SETTINGS.risk_per_trade_usdt)
...
async def _build_tp_targets(
    *,
    ...
    for rr, sz_dec, tag in tp_sizes:
        if sz_dec <= 0:
            # Log when TP levels are skipped due to small position size
            logger.debug(
                "TP level %s skipped for %s: calculated_size=%s (too small after rounding to lotStep=%s)",
                tag, inst_id, sz_dec, lot_step
            )
            continue
...
def _compute_trailing_sl(
    *,
    ...
    last_price: float,
    trail_start_r: float,
    trail_back_r: float | None = None,
) -> float | None:
    """Compute trailing SL level when position is sufficiently in profit.

    trail_start_r: minimum profit (in R) required to activate trailing.
    trail_back_r:  how far behind current price the SL trails (in R).
                   e.g. trail_back_r=0.75 means SL = current_price - 0.75R (for long).
    """
    ...
    if profit_r < trail_start_r:
        return None
    # Continuous trailing: lock in (profit_r - trail_back_r) × R from entry.
    back_r = trail_back_r if trail_back_r is not None else 0.75
    locked_r = max(0.0, profit_r - back_r)
    if side == "buy":
        return entry_price + locked_r * r_value
    else:
        return entry_price - locked_r * r_value
...
def _start_telegram_control() -> None:
    ...
    def command_handler(cmd: str, args: list[str]) -> None:
        ...
        if cmd in {"long", "short"}:
            # /long BTC 1h  OR  /long BTC-USDT-SWAP 1h
            if len(args) < 2:
                notify_info(f"用法: /{cmd} BTC 1h")
                return
            inst_id = _normalize_inst_id(args[0])
            tf = args[1].lower()
            side = "buy" if cmd == "long" else "sell"
            _handle_telegram_trade(inst_id, tf, side)
            return

        if cmd == "close":
            # /close BTC [TF]  OR  /close BTC-USDT-SWAP [TF]
            if len(args) < 1:
                notify_info("用法: /close BTC [TF]")
                return
            inst_id = _normalize_inst_id(args[0])
            tf = args[1].lower() if len(args) >= 2 else "1h"
            _handle_telegram_close(inst_id, tf)
            return
...
async def _refresh_okx_protection() -> None:
    """Trailing-stop refresh for OKX: tightens the algo SL order when position is sufficiently in profit."""
    await asyncio.sleep(15)  # allow startup to settle
    while True:
        await asyncio.sleep(SETTINGS.okx_refresh_seconds)
        if not SETTINGS.trading_enabled or not SETTINGS.okx_refresh_enabled:
            continue
        try:
            await _refresh_okx_trailing()
        except Exception as e:
            logger.warning("okx_refresh_error err=%s", str(e))


async def _refresh_okx_trailing() -> None:
    for inst_id in list(allowed_symbols):
        if inst_id == "*":
            continue
        for pos_side in ("long", "short"):
            ...
            if entry_info is None or entry_info.side != side:
                continue
            ...
            trail_sl = _compute_trailing_sl(
                ...
                trail_back_r=SETTINGS.trail_back_r,
            )
            if trail_sl is None:
                continue
            ...
            # Guard: don't place SL that would trigger immediately against current price
            gap = float(tick_size) if tick_size else 0.0
            if side == "buy" and desired_sl >= last_price - gap:
                continue
            if side == "sell" and desired_sl <= last_price + gap:
                continue

            # Get current algo SL orders and check if update is needed
            try:
                algo_orders = await exchange.get_algo_orders(inst_id=inst_id, ord_type="conditional")
            except Exception:
                algo_orders = []

            current_sl_px: float | None = None
            sl_algo_ids: list[str] = []
            for order in algo_orders:
                info = order.get("info") or order
                trigger = info.get("slTriggerPx") or info.get("triggerPx")
                if not trigger:
                    continue
                try:
                    ...
                    continue
                # Identify as SL order: for long SL is below entry, for short SL is above entry
                if side == "buy" and trigger_f >= entry_info.entry_price:
                    continue
                if side == "sell" and trigger_f <= entry_info.entry_price:
                    continue
                algo_id = info.get("algoId") or order.get("id") or order.get("ordId")
                if algo_id:
                    sl_algo_ids.append(str(algo_id))
                if current_sl_px is None:
                    current_sl_px = trigger_f
                else:
                    # Keep the most favorable existing SL
                    if side == "buy":
                        current_sl_px = max(current_sl_px, trigger_f)
                    else:
                        current_sl_px = min(current_sl_px, trigger_f)

            # Only update if trailing SL is better (tighter) than existing
            if current_sl_px is not None:
                if side == "buy" and desired_sl <= current_sl_px:
                    continue
                if side == "sell" and desired_sl >= current_sl_px:
                    continue

            # Cancel old SL algo orders then place new trailing SL
            for algo_id in sl_algo_ids:
                try:
                    await exchange.cancel_algo_order(inst_id=inst_id, algo_id=algo_id)
                except Exception:
                    pass

            pos_sz = float(pos.get("pos", "0") or "0")
            try:
                inst_info2 = await exchange.get_instrument_info(inst_id=inst_id)
                lot_step = Decimal(str(inst_info2.get("lotStep"))) if inst_info2 and inst_info2.get("lotStep") else None
                min_order = Decimal(str(inst_info2.get("lotSz"))) if inst_info2 and inst_info2.get("lotSz") else None
            except Exception:
                lot_step = min_order = None
            sz_dec = _normalize_qty(Decimal(str(pos_sz)), step=lot_step, min_sz=min_order)
            sz_str = _format_decimal(sz_dec) if sz_dec > 0 else str(pos_sz)
            ...
            try:
                sl_resp = await exchange.place_algo_order(
                    ...
                    ord_type="conditional",
                    sz=sz_str,
                    sl_trigger_px=_round_price_to_tick(desired_sl, tick_size),
                    sl_ord_px="-1",
                )
            except Exception as e:
                sl_resp = {"code": "1", "msg": str(e)}
            ...
            if str(sl_resp.get("code", "")) not in {"0", "success"}:
                _log_decision(inst_id, "", action="okx_trail_sl_failed", desired_sl=desired_sl, resp=sl_resp)
            else:
                profit_r = abs(last_price - entry_info.entry_price) / max(abs(entry_info.entry_price - entry_info.stop_loss), 1e-8)
                _log_decision(inst_id, "", action="okx_trail_sl_updated",
                              side=side, entry=entry_info.entry_price,
                              old_sl=current_sl_px, new_sl=desired_sl, profit_r=round(profit_r, 2))
                notify_info(
                    f"trail SL updated {inst_id} {side} entry={entry_info.entry_price:.4f} "
                    f"new_sl={desired_sl:.4f} profit={profit_r:.1f}R"
                )
    ...
    while True:
        ...
        for (inst_id, pos_side), raw_pos in positions_by_key.items():
            ...
            try:
                ...
                size = Decimal("0")
            if size <= 0:
                _log_decision(inst_id, "", action="grvt_refresh_skip", reason="pos_size_lte_zero", sz=str(size))
                continue
            ...
            if entry_price <= 0:
                try:
                    ...
                    entry_price = 0.0
            if entry_price <= 0:
                _log_decision(inst_id, "", action="grvt_refresh_skip", reason="entry_price_lte_zero")
                continue
            ...
            try:
                ...
                logger.warning("GRVT get_instrument_info failed for %s: %s", inst_id, e)
                inst_info = None
            try:
                inst_info_str = str(inst_info)
                tick_size = inst_info.get("tickSz") if inst_info else None
                lot_step = Decimal(str(inst_info.get("lotStep"))) if inst_info and inst_info.get("lotStep") else None
                min_order = Decimal(str(inst_info.get("lotSz"))) if inst_info and inst_info.get("lotSz") else None
            except Exception as e:
                logger.warning("GRVT inst_info parse failed for %s: %s info=%s", inst_id, e, inst_info_str)
                tick_size = None
                lot_step = None
                min_order = None
            ...
            normalized_sz = _normalize_qty(size, step=lot_step, min_sz=min_order)
            if normalized_sz <= 0:
                _log_decision(inst_id, "", action="grvt_refresh_skip", reason="normalized_sz_lte_zero", sz=str(size), norm=str(normalized_sz))
                continue
            ...
            if entry_info is not None and entry_info.side == side and entry_info.stop_loss > 0:
                ...
                try:
                    ...
                    logger.warning("GRVT get_open_orders failed for %s: %s", inst_id, e)
                    existing_orders = []
                ...
                for order in existing_orders or []:
                    ...
                    def _classify_trigger(px: Decimal, *, trig_type_val: str) -> str | None:
                        ...
                        if take_profit_px is not None and stop_loss_px is None:
                            return "tp"
                        # Handle UNSPECIFIED: infer from price relationship
                        if t == "UNSPECIFIED" or not t:
                            try:
                                px_float = float(px)
                                # If trigger price is below entry, it's likely SL for long (or TP for short)
                                # If trigger price is above entry, it's likely TP for long (or SL for short)
                                if pos_side == "long":
                                    return "sl" if px_float < entry_price else "tp"
                                else:
                                    return "sl" if px_float > entry_price else "tp"
                            except Exception as e:
                                logger.warning(
                                    "Failed to infer trigger type for UNSPECIFIED order: inst=%s px=%s entry=%s error=%s",
                                    inst_id, px, entry_price, e
                                )
                                pass
                        return None
                    ...
                    if trigger_kind is None:
                        # Log unclassified orders for debugging
                        _log_decision(
                            inst_id,
                            "",
                            action="grvt_refresh_unclassified_order",
                            trig_type=trig_type,
                            trig_px=str(px_dec),
                            sz=str(sz_dec),
                        )
                ...
                if last_price is not None and last_price > 0 and anchor_sl > 0:
                    try:
                        trail_sl = _compute_trailing_sl(
                            ...
                            trail_back_r=float(SETTINGS.trail_back_r),
                        )
                    except Exception:
                        trail_sl = None
                ...
                if SETTINGS.grvt_refresh_tp_enabled and SETTINGS.tp_enabled:
                    ...
                    for target in tp_targets:
                        ...
                        if best_p is not None:
                             entries = existing_tp_info.get(best_p) or []
                             # If it matches size, it's valid.
                             for e_sz, e_id in entries:
                                 if abs(e_sz - t_size) <= (lot_step or Decimal("0.000001")):
                                     valid_orphan_check_ids.add(e_id)

                    # Cancel extras
                    for price, entries in existing_tp_info.items():
                        for sz, oid in entries:
...
async def _startup() -> None:
    ...
    if SETTINGS.exchange == "okx" and SETTINGS.okx_refresh_enabled:
        _refresh_task = asyncio.create_task(_refresh_okx_protection())
...
async def manual_signal(req: ManualSignalRequest) -> dict:
    ...
    # Apply leverage/margin mode if provided by UI
    if req.lever and req.mgn_mode:
        try:
            mgn = req.mgn_mode  # "isolated" or "cross"
            lev = req.lever
            lev_errors = []
            if mgn == "cross":
                parts = req.instId.split("-")
                ccy = parts[1] if len(parts) >= 2 else "USDT"
                resp = await exchange._request_async(
                    "POST", "/api/v5/account/set-leverage",
                    json_body={"ccy": ccy, "lever": lev, "mgnMode": "cross"},
                    is_trading=True,
                )
                if str(resp.get("code", "")) not in {"0", "success"}:
                    lev_errors.append(f"cross: {resp.get('msg', resp)}")
            else:
                for pos_side in ("long", "short"):
                    resp = await exchange._request_async(
                        "POST", "/api/v5/account/set-leverage",
                        json_body={"instId": req.instId, "lever": lev, "mgnMode": "isolated", "posSide": pos_side},
                        is_trading=True,
                    )
                    if str(resp.get("code", "")) not in {"0", "success"}:
                        lev_errors.append(f"{pos_side}: code={resp.get('code')} {resp.get('msg', '')}")
            if lev_errors:
                logger.warning("manual_signal: set leverage FAILED %s %s %s: %s", req.instId, lev, mgn, lev_errors)
                return {"ok": False, "error": f"设置杠杆失败: {'; '.join(lev_errors)}。如有持仓，OKX 不允许修改杠杆。"}
            logger.info("manual_signal: set leverage %s %s %s OK", req.instId, lev, mgn)
        except Exception as e:
            logger.warning("manual_signal: set leverage failed: %s", e)
    ...
    payload = TvPayload(
        ...
        close=str(req.entry_price) if req.entry_price else None,
        zone=None
    )

    # 复用现有的信号处理逻辑
    result = await _process_payload(payload, allow_no_zone=True, source="manual", entry_price_override=req.entry_price, risk_usdt_override=req.risk_usdt, sl_price_override=req.sl_price)
    if isinstance(result, dict):
...
async def _process_payload(payload: TvPayload, *, allow_no_zone: bool = False, source: str = "tradingview", entry_price_override: float | None = None, risk_usdt_override: float | None = None, sl_price_override: float | None = None) -> dict:
    ...
    # If user provided explicit SL price, use it directly (skip strategy SL computation)
    if sl_price_override and sl_price_override > 0:
        sl = sl_price_override
        _log_decision(inst_id, tf, action="sl_price_override", sl=float(sl))
    elif SETTINGS.stop_method == "pivot":
        sl = stop_loss_price(
            side=side,
            entry_price=float(entry_price),
            candles=candles,
    ...
    if (not SETTINGS.trading_enabled) or is_paper_tf:
        ...
        order_sz, size_meta = await _calculate_order_size(
            ...
            entry_price=float(entry_price),
            tf=tf,
            risk_usdt_override=risk_usdt_override,
        )
        if size_meta:
            from app.config import get_risk_per_trade
            logger.info(
    ...
    # 2.5 检查仓位冲突（避免重复开仓）
    # 手动/UI 信号允许加仓（scale-in），跳过冲突检测
    check_conflict = source not in {"manual", "ui"} and (SETTINGS.exchange != "lighter" or SETTINGS.lighter_block_duplicate_positions)
    if check_conflict:
    ...
    # Entry offset: single limit order placed at an offset from current price toward SL.
    # Always active for UI/manual signals without a user-specified price (autoEntry mode).
    # For TradingView signals, gated by ENTRY_TWO_LIMIT_ENABLED env var.
    # For GRVT it's also gated behind GRVT_ALLOW_ENTRY_TWO_LIMIT.
    _ui_auto_entry = source in {"manual", "ui"} and not (entry_price_override and entry_price_override > 0)
    _use_offset_entry = _ui_auto_entry or SETTINGS.entry_two_limit_enabled
    if _use_offset_entry and (SETTINGS.exchange != "grvt" or SETTINGS.grvt_allow_entry_two_limit):
        ref_price = await _get_ref_price_from_bar(inst_id, bar=SETTINGS.entry_two_limit_ref_bar) or float(entry_price)

        _offset_ok = ref_price > 0
        if not _offset_ok:
            _log_decision(inst_id, tf, action="two_limit_skip", reason="no_ref_price")
            if not _ui_auto_entry:
                return {"ok": True, "skipped": "no_ref_price"}
            # UI/manual: fall through to market order
        else:
            # Compute stop-loss distance from ref price (D).
            if side == "buy":
                d = float(ref_price) - float(sl)
            else:
                d = float(sl) - float(ref_price)
            ...
            if d <= 0:
                ...
                _offset_ok = False  # UI/manual: fall through to market order

        if _offset_ok:
            # If user provided a specific entry price override, use it directly (no further offset).
            if entry_price_override and entry_price_override > 0:
                px_rounded = _round_price_to_tick(entry_price_override, tick_size)
                towards_sl_pct = 0.0
            else:
                # Compute limit price at towards_sl_pct% of SL distance from ref price.
                # Short: limit above current price (toward SL which is above).
                # Long:  limit below current price (toward SL which is below).
                ...
                if side == "buy":
                    px_target = float(ref_price) - (towards_sl_pct * d)
                else:
                    px_target = float(ref_price) + (towards_sl_pct * d)
            ...
            if str(resp.get("code", "")) not in {"0", "success"}:
                _log_decision(inst_id, tf, action="limit_entry_rejected", px=px_rounded, resp=resp)
                return {
                    "ok": False,
                    "type": "DIV",
                    "side": side,
            ...
            fill_tracker.register_order_label(key=cl_ord_id, label="entry_limit")
            symbol_for_state = inst_id.replace("-USDT-SWAP", "/USDT")
            state.add_pending_order(key=key, order_id=cl_ord_id, symbol=symbol_for_state, level="L")
            state.mark_traded(key)
            state.clear_zone(key)
            _log_decision(
                inst_id, tf,
                action="limit_entry_placed",
                side=side,
                posSide=pos_side,
                ref=ref_price,
                sl=sl,
                px=px_rounded,
                towards_sl_pct=towards_sl_pct,
                sz=sz_str,
            )
...
def _handle_telegram_trade(inst_id: str, tf: str, side: str) -> None:
    """Handle /long or /short Telegram command — triggers full SL/TP order logic."""
    import asyncio
    import datetime as dt
    ...
    async def _run() -> None:
        ...
        result = await _process_payload(payload, allow_no_zone=True, source="telegram")
        if result.get("deduped") or result.get("skipped"):
            notify_info(f"⚠️ 信号被跳过: {result.get('deduped') and '重复信号' or result.get('skipped')}")
            return
        if not result.get("ok"):
            notify_info(f"❌ 下单失败: {result}")
            return
        if result.get("paper"):
            inst_id_norm = result.get("instId") or inst_id
            entry = result.get("entry")
            sl = result.get("sl")
            notify_info(
                f"📋 Paper模式\n{inst_id_norm} {tf} {side}\n"
                f"entry={entry} sl={sl} sz={result.get('order_sz')}"
            )

    asyncio.run(_run())


def _handle_telegram_close(inst_id: str, tf: str) -> None:
    """Handle /close Telegram command — market close the position."""
    import asyncio
    import datetime as dt
    ...
    async def _run() -> None:
        ...
        result = await _process_payload(payload, allow_no_zone=True, source="telegram")
        if result.get("skipped") == "no_position":
            notify_info(f"⚠️ {inst_id} 没有持仓")
        elif result.get("ok"):
            notify_info(f"✅ 已发出平仓指令: {inst_id}")
        else:
            notify_info(f"❌ 平仓失败: {result}")

    asyncio.run(_run())


def _handle_telegram_status() -> None:
    """Handle /status Telegram command — show current OKX positions."""
    import asyncio

    async def _run() -> None:
        if SETTINGS.exchange != "okx":
            notify_info(f"当前交易所: {SETTINGS.exchange}，/status 仅支持 OKX 模式")
            return
        try:
            resp = await exchange._request_async(
                "GET", "/api/v5/account/positions", params={"instType": "SWAP"}, is_trading=True
            )
            positions = resp if isinstance(resp, list) else (resp.get("data") or [])
        except Exception as e:
            notify_info(f"❌ 获取持仓失败: {e}")
            return

        active = [p for p in positions if float(p.get("pos", "0") or "0") != 0.0]
        if not active:
            notify_info("📊 当前无持仓")
            return

        lines = [f"📊 *持仓 ({len(active)})*"]
        for p in active:
            base = (p.get("instId") or "").replace("-USDT-SWAP", "").replace("-USD-SWAP", "")
            ps = p.get("posSide", "")
            side_label = "🟢 LONG" if ps == "long" else "🔴 SHORT"
            sz = p.get("pos", "?")
            avg_px = float(p.get("avgPx") or 0)
            upl = float(p.get("uPnl") or p.get("upl") or 0)
            upl_ratio = float(p.get("uplRatio") or 0) * 100
            liq_px = float(p.get("liqPx") or 0)
            lever = p.get("lever", "?")
            pnl_sign = "+" if upl >= 0 else ""
            lines.append(
                f"\n*{base}* {side_label} {lever}×\n"
                f"  开仓: {avg_px:,.2f}  数量: {sz}张\n"
                f"  浮盈: {pnl_sign}{upl:.2f}U ({pnl_sign}{upl_ratio:.2f}%)\n"
                f"  强平: {liq_px:,.2f}"
            )
        notify_info("\n".join(lines))
