...
PerpBot 是一个**高度模块化**的自动交易框架，专注于多交易所套利和自动化交易策略。本项目采用创新的三层资金管理架构，支持 CEX 和 DEX 的统一接入，并提供了从宏观到微观的完整风控体系和实时监控系统。
...
- **模块化架构**: 策略、风控和交易所实现完全解耦，具备极高扩展性。
- **三层资金管理**: 独创的 **刷量/套利/储备** 三层资金池模型，实现动态风险敞口管理。
- **高性能套利引擎**: 亚秒级跨所价差扫描（支持42种配对），内置智能评分与并发执行逻辑。
- **多层纵深风控**: 覆盖从资金层、账户级到仓位级的四层风险防护体系。
- **实时监控控制台**: 基于 FastAPI 和 WebSocket 的 Web UI，提供实时监控、在线调参和一键启停功能。

### 功能矩阵

| 功能模块 | 详细说明 | 状态 |
|---|---|---|
| 🏦 **8个交易所客户端** | Paradex/Extended/Lighter/EdgeX/Backpack/GRVT/Aster/OKX | ✅ 已集成 |
| 💰 **三层资金管理** | 刷量/套利/储备三层资金池动态调度 | ✅ 已实现 |
| 🔄 **高性能套利引擎** | 42种DEX配对扫描 + 智能评分 + 并发执行 | ✅ 已实现 |
| 🛡️ **多层纵深风控** | 覆盖资金层、账户级、仓位级和执行层的四层防护体系 | ✅ 已实现 |
| 🖥️ **Web 监控控制台** | 实时行情、套利机会、资产曲线、在线控制 | ✅ 已实现 |
| 📈 **高级策略模块** | 追踪止损、网格交易、动态仓位管理 | ✅ 已实现 |
| 🔌 **连接与通知** | WebSocket自动重连管理器, 多渠道通知 (Telegram/Lark等) | ✅ 已实现 |
| ⚙️ **配置与测试** | YAML配置验证与热重载, 通用交易所测试框架 | ✅ 已实现 |
...
传统的5层资金管理模型过于复杂，本项目采用**极简三层模型**，专注于在刷量对冲和套利场景中实现风险与收益的平衡：
